...
MmdScen.prototype = {
  ...
  AddCameraControl: function (x, y, z) {
    this.cameraControl = new THREE.OrbitControls(
      this.camera,
      renderer.domElement
    );
...
MmdLight.prototype = {
  ...
  AddLightHelper: function () {
    ...
    if (this.directionalLight) {
      this.directionalLightHleper = new THREE.CameraHelper(
...
function SLOG(str) {
  if (str) {
    ...
    loadingLog += "로그가 없는 원인을 찾아주세요" + "<br/>";
    $("#progressLog").html(loadingLog);
    return false;
  }
}
...
function onProgress(xhr) {
  ...
  if (xhr.lengthComputable) {
    if (xhr.loaded == xhr.total) {
      ...
      SLOG(fileType + ":" + fileName + "불러오기 완료");

      $("#progressTitle").html("");

      if (fileType == "오디오 파일")
        SLOG("모든 파일이 로드됨" + "<br/>" + "파일작업 중...");
    } else {
    ...
    SLOG(
      fileType + ":" + fileName + "로드가 완료되지 않음. 네트워크 확인 필요"
    );
  }
}
    ...
    function (object) {
      ...
      loader.loadVmds(
        ...
        function (vmd) {
          ...
          if (helper.doCameraAnimation) SLOG("자동 카메라 켜기");
          loader.loadAudio(
            audioFile,
            function (audio, listener) {
              ...
              if (myMmdScen.camera.add(listener))
                SLOG("오디오가 카메라에 바인딩됨");

              if (scene.add(mesh)) SLOG("장면에 모델이 추가됨");

              if (mesh.add(audio)) SLOG("모델에 오디오 추가됨");
  ...
  function timedCount() {
    $("#progressLog").html(
      loadingLog +
        "메시지 상자" +
        c +
        "초 후 자동으로 숨깁니다. 설정에서 LOG 버튼 클릭<br/>"
    );
...
function AddControlGui(controlObject) {
  ...
  var mmdSet = gui.addFolder("모델 설정");
  ...
  mmdSet
    .add(controlObject, "gradient_mapping")
    .name("그라데이션 매핑")
    .onChange(function () {
  ...
  mmdSet
    .add(controlObject, "show_IK_bones")
    .name("본 보기")
    .onChange(function () {
  ...
  mmdSet
    .add(controlObject, "show_rigid_bodies")
    .name("강체 보기")
    .onChange(function () {
      if (physicsHelper !== undefined)
        physicsHelper.visible = control.show_rigid_bodies;
    });

  //========================================================

  var sceneSet = gui.addFolder("장면 설정");

  skySunSet = sceneSet.addFolder("하늘 설정");

  skySunSet
    .add(effectControllerSky, "turbidity", 1.0, 20.0, 0.1)
    .onChange(guiSkyChanged)
    .name("흐림");
  skySunSet
    .add(effectControllerSky, "rayleigh", 0.0, 4, 0.001)
    .onChange(guiSkyChanged)
    .name("레일리(밝기)");
  skySunSet
    .add(effectControllerSky, "mieCoefficient", 0.0, 0.1, 0.001)
    .onChange(guiSkyChanged)
    .name("계수");
  skySunSet
    .add(effectControllerSky, "mieDirectionalG", 0.0, 1, 0.001)
    .onChange(guiSkyChanged)
    .name("산란");
  skySunSet
    .add(effectControllerSky, "luminance", 0.0, 2)
    .onChange(guiSkyChanged)
    .name("휘도");
  skySunSet
    .add(effectControllerSky, "inclination", 0, 1, 0.0001)
    .onChange(guiSkyChanged)
    .name("기울기");
  skySunSet
    .add(effectControllerSky, "azimuth", 0, 1, 0.0001)
    .onChange(guiSkyChanged)
    .name("방위각");
  skySunSet
    .add(effectControllerSky, "sun")
    .onChange(guiSkyChanged)
    .name("태양");

  sceneSet
    .add(controlObject, "sky")
    .name("하늘")
  ...
  sceneSet
    .add(controlObject, "plane", ["거울면", "보통", "없음"])
    .name("지면")
    .onChange(function (value) {
      if (value == "거울면") {
  ...
  sceneSet
    .add(controlObject, "gridHelper", ["직사각형", "원", "없음"])
    .name("격자")
    .onChange(function (value) {
      if (value == "직사각형") {
  ...
  var lightColorSet = gui.addFolder("빛 설정");

  lightColorSet
    .add(controlObject, "castShadow")
    .name("Light Shadow")
    .onChange(function (value) {
      if (value) {
  ...
  //--------------------------------------------------------------
  lightColorSet
    .add(controlObject, "spotLight")
    .name("Spot Light")
    .onChange(function (value) {
      if (value)
        myMmdLight.AddSpotLight(new THREE.Color(controlObject.spotLightSet));
      else myMmdLight.RemoveSpotLight();
    });

  lightColorSet
    .addColor(controlObject, "spotLightSet")
    .name("spot LightSet")
    .onChange(function () {
      if (myMmdLight.spotLight) {
        myMmdLight.spotLight.color = new THREE.Color(
          controlObject.spotLightSet
        );
      }
    });
  ...
  //--------------------------------------------------------------
  lightColorSet
    .add(controlObject, "directionalLight")
    .name("Directional Light")
    .onChange(function (value) {
      if (value)
        myMmdLight.AddDirectionalLight(
          new THREE.Color(controlObject.directionalLightSet)
        );
      else myMmdLight.RemoveDirectionalLight();
    });

  lightColorSet
    .addColor(controlObject, "directionalLightSet")
    .name("directional LightSet")
    .onChange(function () {
      if (myMmdLight.directionalLight) {
        myMmdLight.directionalLight.color = new THREE.Color(
          controlObject.directionalLightSet
        );
      }
    });
  ...
  lightColorSet
    .add(controlObject, "ambientLight")
    .name("Ambient Light")
    .onChange(function (value) {
      if (value) {
        myMmdLight.AddAmbientLight(
          new THREE.Color(controlObject.ambientLightSet)
        );
      } else {
  ...
  lightColorSet
    .addColor(controlObject, "ambientLightSet")
    .name("ambient LightSet")
    .onChange(function () {
      if (myMmdScen.ambientLight) {
        myMmdScen.ambientLight.color = new THREE.Color(
          controlObject.ambientLightSet
        );
      }
    });
  ...
  //--------------------------------------------------------------
  lightColorSet
    .addColor(controlObject, "scenBackColor")
    .name("scen BackColor")
    .onChange(function () {
