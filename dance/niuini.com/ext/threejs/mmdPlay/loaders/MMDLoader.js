...
THREE.MMDLoader.prototype.load = function (
  modelUrl,
  vmdUrls,
  callback,
  onProgress,
...
THREE.MMDLoader.prototype.loadModel = function (
  url,
  callback,
  onProgress,
  onError
...
THREE.MMDLoader.prototype.createModel = function (
  buffer,
  modelExtension,
  texturePath,
  onProgress,
  ...
  return this.createMesh(
    this.parseModel(buffer, modelExtension),
    texturePath,
    onProgress,
    onError
  );
...
THREE.MMDLoader.prototype.loadVmd = function (
  url,
  callback,
  onProgress,
  onError
...
THREE.MMDLoader.prototype.loadVmds = function (
  urls,
  callback,
  onProgress,
  onError
...
THREE.MMDLoader.prototype.loadAudio = function (
  url,
  callback,
  onProgress,
  onError
...
THREE.MMDLoader.prototype.loadVpd = function (
  url,
  callback,
  onProgress,
  onError,
  ...
  var func = (
    params && params.charcode === "unicode"
      ? this.loadFileAsText
      : this.loadFileAsShiftJISText
  ).bind(this);
...
THREE.MMDLoader.prototype.pourVmdIntoCamera = function (camera, vmd, name) {
  ...
  var initAnimation = function () {
    ...
    tracks.push(
      createTrack(".fov", "NumberKeyframeTrackEx", times, fovs, fInterpolations)
    );

    var clip = new THREE.AnimationClip(
      name === undefined ? THREE.Math.generateUUID() : name,
      -1,
      tracks
    );
...
THREE.MMDLoader.prototype.loadFile = function (
  url,
  onLoad,
  onProgress,
  onError,
...
THREE.MMDLoader.prototype.loadFileAsBuffer = function (
  url,
  onLoad,
  onProgress,
  onError
...
THREE.MMDLoader.prototype.loadFileAsText = function (
  url,
  onLoad,
  onProgress,
  onError
...
THREE.MMDLoader.prototype.loadFileAsShiftJISText = function (
  url,
  onLoad,
  onProgress,
  onError
...
THREE.MMDLoader.prototype.createMesh = function (
  model,
  texturePath,
  onProgress,
  onError
  ...
  var initVartices = function () {
    for (var i = 0; i < model.metadata.vertexCount; i++) {
      ...
      for (var j = 0; j < 4; j++) {
        buffer.skinIndices.push(
          v.skinIndices.length - 1 >= j ? v.skinIndices[j] : 0.0
        );
      }

      for (var j = 0; j < 4; j++) {
        buffer.skinWeights.push(
  ...
  var initMaterials = function () {
    ...
    function loadTexture(filePath, params) {
      ...
      if (params.defaultTexturePath === true) {
        try {
          fullPath =
            scope.defaultToonTextures[
              parseInt(filePath.match("toon([0-9]{2}).bmp$")[1])
            ];
        } catch (e) {
          console.warn(
            "THREE.MMDLoader: " +
              filePath +
              " seems like not right default texture path. Using toon00.bmp instead."
          );
    ...
    for (var i = 0; i < materialParams.length; i++) {
      ...
      m.lights = true;
      m.side =
        model.metadata.format === "pmx" && (p2.flag & 0x1) === 1
          ? THREE.DoubleSide
          : p.side;
      ...
      if (p.map !== undefined) {
        ...
        function checkTextureTransparency(m) {
          m.map.readyCallbacks.push(function (t) {
            ...
            var imageData =
              t.image.data !== undefined ? t.image : createImageData(t.image);
            var indices = geometry.index.array.slice(
              m.faceOffset * 3,
              m.faceOffset * 3 + m.faceNum * 3
            );
      ...
      if (model.metadata.format === "pmd") {
        ...
        var toonFileName =
          p2.toonIndex === -1
            ? "toon00.bmp"
            : model.toonTextures[p2.toonIndex].fileName;
  ...
  var initGeometry = function () {
    ...
    geometry.addAttribute(
      "skinIndex",
      new THREE.Float32BufferAttribute(buffer.skinIndices, 4)
    );
    geometry.addAttribute(
      "skinWeight",
      new THREE.Float32BufferAttribute(buffer.skinWeights, 4)
    );
...
THREE.MMDLoader.prototype.createAnimation = function (mesh, vmd, name) {
  ...
  var initMotionAnimations = function () {
    ...
    var orderedMotions = helper.createOrderedMotionArrays(
      bones,
      vmd.motions,
      "boneName"
    );
    ...
    for (var i = 0; i < orderedMotions.length; i++) {
      ...
      tracks.push(
        new THREE.MMDLoader.VectorKeyframeTrackEx(
          boneName + ".position",
          times,
          positions,
          pInterpolations
      ...
      tracks.push(
        new THREE.MMDLoader.QuaternionKeyframeTrackEx(
          boneName + ".quaternion",
          times,
          rotations,
          rInterpolations
  ...
  var initMorphAnimations = function () {
    ...
    var orderedMorphs = helper.createOrderedMotionArrays(
    ...
    var clip = new THREE.AnimationClip(
      name === undefined ? THREE.Math.generateUUID() : name + "Morph",
      -1,
      tracks
    );
 ...
 */
THREE.MMDLoader.VectorKeyframeTrackEx = function (
  name,
  times,
  values,
...
THREE.MMDLoader.VectorKeyframeTrackEx.prototype = Object.create(
  THREE.VectorKeyframeTrack.prototype
);
THREE.MMDLoader.VectorKeyframeTrackEx.prototype.constructor =
  THREE.MMDLoader.VectorKeyframeTrackEx;
...
THREE.MMDLoader.VectorKeyframeTrackEx.prototype.InterpolantFactoryMethodCubicBezier =
  function (result) {
    return new THREE.MMDLoader.CubicBezierInterpolation(
      this.times,
      this.values,
      this.getValueSize(),
      result,
...
THREE.MMDLoader.VectorKeyframeTrackEx.prototype.setInterpolation = function (
  interpolation
) {
  this.createInterpolant = this.InterpolantFactoryMethodCubicBezier;
};

THREE.MMDLoader.QuaternionKeyframeTrackEx = function (
  name,
  times,
  values,
  interpolationParameterArray
...
THREE.MMDLoader.QuaternionKeyframeTrackEx.prototype = Object.create(
  THREE.QuaternionKeyframeTrack.prototype
);
THREE.MMDLoader.QuaternionKeyframeTrackEx.prototype.constructor =
  THREE.MMDLoader.QuaternionKeyframeTrackEx;
THREE.MMDLoader.QuaternionKeyframeTrackEx.prototype.TimeBufferType =
  Float64Array;
...
THREE.MMDLoader.QuaternionKeyframeTrackEx.prototype.setInterpolation =
  function (interpolation) {
    this.createInterpolant = this.InterpolantFactoryMethodCubicBezier;
  };

THREE.MMDLoader.NumberKeyframeTrackEx = function (
  name,
  times,
  values,
  interpolationParameterArray
...
THREE.MMDLoader.NumberKeyframeTrackEx.prototype = Object.create(
  THREE.NumberKeyframeTrack.prototype
);
THREE.MMDLoader.NumberKeyframeTrackEx.prototype.constructor =
  THREE.MMDLoader.NumberKeyframeTrackEx;
THREE.MMDLoader.NumberKeyframeTrackEx.prototype.TimeBufferType = Float64Array;

THREE.MMDLoader.NumberKeyframeTrackEx.prototype.InterpolantFactoryMethodCubicBezier =
  function (result) {
    return new THREE.MMDLoader.CubicBezierInterpolation(
      this.times,
      this.values,
...
THREE.MMDLoader.NumberKeyframeTrackEx.prototype.setInterpolation = function (
  interpolation
) {
  this.createInterpolant = this.InterpolantFactoryMethodCubicBezier;
};

THREE.MMDLoader.CubicBezierInterpolation = function (
  parameterPositions,
  sampleValues,
  sampleSize,
  resultBuffer,
...
THREE.MMDLoader.CubicBezierInterpolation.prototype = Object.create(
  THREE.LinearInterpolant.prototype
);
THREE.MMDLoader.CubicBezierInterpolation.prototype.constructor =
  THREE.MMDLoader.CubicBezierInterpolation;

THREE.MMDLoader.CubicBezierInterpolation.prototype.interpolate_ = function (
  i1,
  t0,
  t,
  t1
...
THREE.MMDLoader.CubicBezierInterpolation.prototype._calculate = function (
  x1,
  x2,
  y1,
  y2,
...
THREE.MMDHelper.prototype = {
  ...
  add: function (mesh) {
    if (!(mesh instanceof THREE.SkinnedMesh)) {
      throw new Error(
        "THREE.MMDHelper.add() accepts only THREE.SkinnedMesh instance."
      );
    }
  ...
  setPhysics: function (mesh, params) {
    ...
    if (
      mesh.mixer !== null &&
      mesh.mixer !== undefined &&
      params.preventAnimationWarmup !== true
    ) {
  ...
  setAnimation: function (mesh) {
    if (mesh.geometry.animations !== undefined) {
      ...
      mesh.mixer.addEventListener("loop", function (e) {
        if (
          e.action._clip.tracks.length > 0 &&
          e.action._clip.tracks[0].name.indexOf(".bones") !== 0
        )
          return;
      ...
      for (var i = 0; i < mesh.geometry.animations.length; i++) {
        ...
        if (
          ...
          clip.tracks[0].name.indexOf(".morphTargetInfluences") === 0
        ) {
  ...
  unifyAnimationDuration: function (params) {
    ...
    //**addif()
    try {
      var tempstr = "전체모듈 최대시간:" + Math.round(max) + "s";
