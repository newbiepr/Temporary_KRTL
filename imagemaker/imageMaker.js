...
(function (global, $) {
  ...
  if (typeof $ === 'undefined') {
        console.error('It requires jQuery.');
}
...
var initialElements  = function(canvas_info) {
    ...
    $.each(canvas_info.templates2,function(delta, img){
        buildImagesMemes += '<div class="wrapper_select_image_amm">';
        buildImagesMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.template_thumbnail_width+'" height="'+canvas_info.template_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
        buildImagesMemes += '</div>';
    });
    ...
    $.each(canvas_info.templates3,function(delta, img){
        buildImagesMemes += '<div class="wrapper_select_image_amm">';
        buildImagesMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.template_thumbnail_width+'" height="'+canvas_info.template_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
        buildImagesMemes += '</div>';
    });
    ...
    $.each(canvas_info.templates4,function(delta, img){
        buildImagesMemes += '<div class="wrapper_select_image_amm">';
        buildImagesMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.template_thumbnail_width+'" height="'+canvas_info.template_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
        buildImagesMemes += '</div>';
    });
    ...
    $.each(canvas_info.templates5,function(delta, img){
        buildImagesMemes += '<div class="wrapper_select_image_amm">';
        buildImagesMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.template_thumbnail_width+'" height="'+canvas_info.template_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
        buildImagesMemes += '</div>';
    });
    ...
    var buildImagesMergeMemes = '<div class="wrapper-amm-select-merge-images">';
     ...
     buildImagesMergeMemes +='</div>'
        ...
        buildImagesMergeMemes +='<div class="wrapper-thumbnail-merge-images">';
        $.each(canvas_info.merge_images2,function(delta, img){
            buildImagesMergeMemes +='<div class="wrapper-select-merge-image-amm">';
            buildImagesMergeMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.merge_image_thumbnail_width+'" height="'+canvas_info.merge_image_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
            buildImagesMergeMemes +='</div>';
                });
     buildImagesMergeMemes +='</div>'
        ...
        buildImagesMergeMemes +='<div class="wrapper-thumbnail-merge-images">';
        $.each(canvas_info.merge_images3,function(delta, img){
            buildImagesMergeMemes +='<div class="wrapper-select-merge-image-amm">';
            buildImagesMergeMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.merge_image_thumbnail_width+'" height="'+canvas_info.merge_image_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
            buildImagesMergeMemes +='</div>';
                });
     buildImagesMergeMemes +='</div>'
        ...
        buildImagesMergeMemes +='<div class="wrapper-thumbnail-merge-images">';
        $.each(canvas_info.merge_images4,function(delta, img){
            buildImagesMergeMemes +='<div class="wrapper-select-merge-image-amm">';
            buildImagesMergeMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.merge_image_thumbnail_width+'" height="'+canvas_info.merge_image_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
            buildImagesMergeMemes +='</div>';
                });
     buildImagesMergeMemes +='</div>'
        ...
        buildImagesMergeMemes +='<div class="wrapper-thumbnail-merge-images">';
        $.each(canvas_info.merge_images5,function(delta, img){
            buildImagesMergeMemes +='<div class="wrapper-select-merge-image-amm">';
            buildImagesMergeMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.merge_image_thumbnail_width+'" height="'+canvas_info.merge_image_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
            buildImagesMergeMemes +='</div>';
                });
     buildImagesMergeMemes +='</div>'
        ...
        buildImagesMergeMemes +='<div class="wrapper-thumbnail-merge-images">';
        $.each(canvas_info.merge_images6,function(delta, img){
            buildImagesMergeMemes +='<div class="wrapper-select-merge-image-amm">';
            buildImagesMergeMemes +=   '<img typeof="foaf:Image" width="'+canvas_info.merge_image_thumbnail_width+'" height="'+canvas_info.merge_image_thumbnail_height+'" src="'+img.url+'" title="'+img.title+'">';
            buildImagesMergeMemes +='</div>';
                });
...
$(this).each(function (delta) {
     ...
     var canvas_info = {
        ...
        _maxWidth: 1136,//The initial size that used for scale
        _maxHeight: 640,
        position_left: 0,
        ratio_image: 1,
        ratio_width: 0,
        ...
        merge_images5: [],
        merge_images6: [],
        merge_image_thumbnail_width:50,
        ...
        i18n:{fontFamilyText: '폰트',
             enterTextText:'텍스트 추가',
             topText:'이름 텍스트',
             bottomText: '커뮤 첫번째줄',
			 bottomText2: '커뮤 두번째줄',
             sizeText:'폰트 크기',
             uperCaseText:'대문자',
             mergeImageText: '이미지 추가',
             drawText:'그리기',
             addTextBoxText:'텍스트 추가',
             previewText:'미리보기',
             addTemplateText:'배경 추가',
             resetText: '초기화',
             imageGeneratorText: '이미지 다운로드',
             stopBrushingText:'그리기 중단',
             canvasLoadingText: '로딩중'
              },
  ...
  BrushesPoints.prototype.reset = function(){
    ...
    var brushespoints = new BrushesPoints();
     ...
     var ImageInfo = function(){
         ...
         this.top = 0;
         this.left = 0;
         this.height = 146; //해당 옵션값 무시됨
         this.width = 722;  //해당 옵션값 무시됨
         this.ratioWidth=this.width/canvas_info.final_width;   //해당 옵션값 무시됨
         this.ratioHeight=this.height/canvas_info.final_height;   //해당 옵션값 무시됨
         this.ratioLeft = 0.172;
         this.ratioTop = 0.74;
         this.img = null;
     };
     ...
     container.on('click', ".add_textbox", function(event) {
          event.preventDefault();
           ...
           var text_info = {
                ...
                id_box: null,
                delta: null,
                ratioWidth:0.3, //used for responsive aspect
                ratioHeight: 0.1, 
                ratioLeft: 0.2,
                ratioTop:0,
                text:"",
        ...
        //    console.log("texts", texts);
           ...
           var delta = texts.length;
            ...
            var style = "";
            var top = container.find(selectors.ammCanvas).height() - (container.find(selectors.ammCanvas).outerHeight()*text_info.ratioHeight);
            if(delta == 0){ text_info.placeholder = canvas_info.i18n.topText;
                text_info.ratioTop = 0.74 
				text_info.ratioLeft = 0.21
            }
            if(delta == 1){
                text_info.placeholder = canvas_info.i18n.bottomText;
                //       Height Canvas             -       Responsive Height of Box
			    text_info.ratioTop = 0.81
				text_info.ratioWidth = 0.6
//                console.log(top);
            }
            if(delta == 2){
                text_info.placeholder = canvas_info.i18n.bottomText2;
                //       Height Canvas             -       Responsive Height of Box
			    text_info.ratioTop = 0.86
				text_info.ratioWidth = 0.6
//                console.log(top);
            }
            if(delta > 2){
...
//                    
            ...
            .resizable({
                ...
                minWidth: 50,
                    ...
                    {border: "0px dashed white", position: "absolute", width: canvas_info.final_width + "px", height: canvas_info.height_div + "px"}
            ).animate({
              opacity: 0
             }, 1000, function() {
...
function amm_add_image(event){
    ...
    reader.onload = function(event) {
		...
		image_info.width = image_info.img.width
        image_info.height = image_info.img.height
        image_info.ratioHeight=image_info.height/canvas_info.final_height; 
        image_info.ratioWidth=image_info.width/canvas_info.final_width; 
...
// console.log([ui.position.top, $(window).scrollTop()]);
            ...
            stop: function(event, ui) {
                ...
                {border: "0px dashed white", position: "absolute", width: image_info.width + "px", height: image_info.height + "px"}
        ).animate({
            opacity: 0
        }, 1000, function() {
...
//         image_object.height = image_object.img.height; //으악
//         image_object.width = image_object.img.width;
//         image_object.ratioWidth=image_object.width/canvas_info.final_width; 
//         image_object.ratioHeight=image_object.height/canvas_info.final_height; 
...
//        http://www.w3schools.com/tags/canvas_font.asp
         ...
         // shadow
         //canvas_info.ctx.shadowBlur = text_object.shadowBlur;      //shadow 제거
         //canvas_info.ctx.shadowColor = text_object.shadowColor;
		 
         // outline 
         //canvas_info.ctx.strokeStyle= text_object.strokeColor;          //outline 제거
...
//            console.log([maxWidth, canvas_info.final_width*text_object.ratioWidth, canvas_info.final_width, text_object.ratioWidth]);
        ...
        MemeGeneratorAPI.wrapText(canvas_info.ctx, text, left, top , maxWidth, maxHeight, text_object);
...
MemeGeneratorTheme.wrapper_text = function (text_info) { 
    ...
    var All_FontFamilies = ['Humming-C', 'Arial', 'serif', 'sans-serif'];   
